...
class AuthActivity : ComponentActivity() {
            ...
            if (emailEditText.text.isNotEmpty() && passwordEditText.text.isNotEmpty()) {
                FirebaseAuth.getInstance().createUserWithEmailAndPassword(
                    ...
                    .addOnCompleteListener() {
                        if (it.isSuccessful) {
                            showHome(it.result.user?.email.toString() ?: "", ProviderType.BASIC, it.result.user?.displayName.toString() ?: "")
                        } else {
                            showAlert()
                        }
                    }
        ...
        loginButton.setOnClickListener {
            if (emailEditText.text.isNotEmpty() && passwordEditText.text.isNotEmpty()) {
                FirebaseAuth.getInstance().signInWithEmailAndPassword(
                    ...
                    .addOnCompleteListener() {
                        if (it.isSuccessful) {
                            showHome(it.result.user?.email.toString() ?: "", ProviderType.BASIC, it.result.user?.email.toString() ?: "")
                        } else {
                            showAlert()
                        }
                    }
    ...
    private fun showHome(email: String, provider: ProviderType, name: String){
        ...
        //val homeIntent = Intent(this, DenunciaListFragment::class.java).apply{
            ...
            try {
                ...
                if (account != null) {
                    ...
                    FirebaseAuth.getInstance().signInWithCredential(credential)
                        .addOnCompleteListener() {
                            if (it.isSuccessful) {
                                showHome(account.email ?: "", ProviderType.GOOGLE, account.displayName.toString())
                            } else {
                                showAlert()
                            }
                        }
